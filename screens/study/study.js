...
// ════════════════════════════════════════════════
// 학습 시작 흐름:
//   tryStartStudy → (pendingSession 있으면) 이어하기 모달
//                 → (학습 이력 있으면) 필터 모달 → startWithFilter
//                 → (첫 학습) startFresh
// 첫 세션은 순서대로, 2회차부터 랜덤 셔플 (totalSessions 기준)

async function tryStartStudy(deckId) {
  const deck = state.decks.find(d => (d.deck_id||d.id) === deckId);
  state.pendingDeckId = deckId;

  // 카드가 아직 로드 안 됐으면 lazy load
...
function startWithFilter() {
  ...
  const filtered = deck.cards.filter(c => {
    const s = (state.cardProgress[c.card_id||c.id]||{}).status || null;
    return selected.includes(s);
  }).map(c => ({...c, id: c.card_id||c.id}));
  if (!filtered.length) { showToast('해당하는 카드가 없어요'); return; }
  closeFilterModal();
  deck.pending_session = null;
  saveDeckMeta(deck.deck_id||deck.id, {pending_session: null});
...
function resumeStudy() {
  ...
  const p = deck.pending_session;
  state.studyDeck = deck;
  state.studyQueue = p.remainingIds.map(id => deck.cards.find(c => (c.card_id||c.id) === id)).filter(Boolean)
    .map(c => ({...c, id: c.card_id||c.id}));
...
function exitStudy() {
  if (!state._starredMode && state.studyDeck && state.studyIdx < state.studyQueue.length) {
    ...
    saveDeckMeta(deckId, { pending_session: pending, last_studied: today });
  }
...
function showDone() {
  if (!state._starredMode) {
    ...
    state.studyDeck.total_sessions = newTotal;
    state.studyDeck.last_studied = today;
    state.studyDeck.pending_session = null;
  }
  state._starredMode = false;
  showScreen('done');
...
async function saveEditCard() {
  ...
  const deck = state.decks.find(d => (d.deck_id||d.id) === deckId);
  if (deck) {
    const idx = deck.cards.findIndex(c => (c.card_id||c.id) === cardId);
  ...
  showLoading('저장 중...');
  try {
    await sbUpdate('cards', `card_id=eq.${cardId}`, { front, back, hint: hint||null });
    showToast('저장 완료!');
